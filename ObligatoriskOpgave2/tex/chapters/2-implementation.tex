...
Arbejdet hver tråd skal udføre er delt ud i 'bidder' af arbejde i form af en \texttt{thread\_data} struct. Denne struct indeholder tre tal: \texttt{from}, der indikerer ved hvilken iteration summeringen skal starte, \texttt{to}, der indikerer ved hvilken iteration summeringen skal slutte og \texttt{sum}, der er resultatet af summeringen af intervallet i structen. Der genereres en struct for hver tråd, og disse gemmes herefter i et array.\\
...
\texttt{pthread\_create} tager imod en \texttt{SOMETHING}, en \texttt{SOMETHING}, en funktions-pointer, og et eventuelt parameter. \\

\texttt{pthread\_join} tager imod et \texttt{pthread\_t} tråd-ID, samt en \texttt{SOMETHING} og gør noget.\\
...
This function adds Node *n to the list *l.
\texttt{}
